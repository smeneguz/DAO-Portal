...
// lib/hooks/useDAOs.ts - Improved version with robust error handling and fallback
  ...
  useEffect(() => {
    const fetchDAOs = async () => {
      ...
      try {
        ...
        // Fetch from the API - no need for local JSON fallback since API now includes metrics
        ...
        // Transform the API data into our DAO format - now using API metrics data
        const transformedData = apiData.items.map((dao: any) => {
          ...
          // Create a transformed DAO object using API data directly (no need for local fallback)
          return {
            id: dao.id,
            ...
            // Use metrics data directly from API response
